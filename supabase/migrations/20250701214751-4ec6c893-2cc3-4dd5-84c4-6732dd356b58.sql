-- Fix complete_franchise_onboarding function to handle empty strings properly
...
-- Fix update_franchise_dashboard_access function to be more robust
...
BEGIN
  ...
  IF target_member_id IS NULL THEN
    RAISE EXCEPTION 'Member ID cannot be null';
  END IF;
  ...
  SELECT EXISTS(SELECT 1 FROM franchise_members WHERE id = target_member_id) INTO member_exists;
  
  IF NOT member_exists THEN
    RAISE EXCEPTION 'Franchise member not found with ID: %', target_member_id;
  END IF;
...
-- Fix get_franchise_member_by_order function to handle missing data
